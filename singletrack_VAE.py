...
class db_processing:
    ...
    def melodies_from_song(self, song_data):
    ...
    def melodies_from_song_lakh(self, song_data):
    ...
    def song_from_melody(self, melody):

        zeros = np.zeros((3, melody.shape[0]), dtype=np.int32)
        song = [melody, zeros]
        song_data = np.vstack(song)
...
class singletrack_vae:
    ...
    def encode_sequence(self, melodies):

        num_melodies = len(melodies)
        song_data = np.vstack(melodies)
        ...
        z_split_melodies = np.split(z, num_melodies, axis=0)

        return z_split_melodies

    def decode_sequence(self, z, total_steps, temperature):

        # dec = self.model.decode(z, total_steps, temperature)

        num_z_melodies = len(z)
        z_stacked = np.vstack(z)

        song_tensors = self.model.decode_to_tensors(z_stacked, total_steps, temperature, None, False)

        num_double_measures = len(song_tensors)//num_z_melodies
        num_measures = num_double_measures * 2
        num_timesteps = num_measures * 16

        song_data = np.zeros((num_z_melodies, num_timesteps), dtype=np.int32)
        # song_data.fill(-1)

        for i in range(num_z_melodies):
            for j in range(num_timesteps):
                    token = np.argmax(song_tensors[num_double_measures * i + (j // total_steps)][j % total_steps])
                    song_data[i, j] = token

        song_data_split = np.split(song_data, num_z_melodies, axis=0)
        song_data_split = [np.squeeze(melody, axis=0) for melody in song_data_split]

        return song_data_split
...
if __name__ == "__main__":
    ...
    song_data = song_data[:, :16*32]
    melodies = db_proc.melodies_from_song(song_data)

    # print(melodies)
    z_melodies = vae.encode_sequence(melodies)

    melodies_r = vae.decode_sequence(z_melodies, total_steps, temperature)

    compare_encoded_decoded = (np.array(melodies) == np.array(melodies_r))
    true_count = np.count_nonzero(compare_encoded_decoded)
    total_elements = compare_encoded_decoded.size
    print(f"accuracy-{true_count / total_elements}")

    song_data = db_proc.song_from_melody(melodies_r[0])
    midi = db_proc.midi_from_song(song_data)
    midi.save("mario_melody.mid")
